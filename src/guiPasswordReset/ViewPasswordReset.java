...
public class ViewPasswordReset {
	...
	private static Label label_TitleLine2 = 
			new Label("Enter your new password and then click on " + 
					"reset password.");
	
	protected static Label label_PasswordsDoNotMatch = new Label();
	protected static Label text_Username = new Label();
	...
	protected static Label canReturn = new Label();
	...
	private static Scene passwordResetScene = null;
	 ...
	 */
	public static void displayPasswordReset(Stage ps, String user) {
		...
		new ViewPasswordReset(user);	// Create an instance of the class
		
		// Set the title for the window, display the page, and wait for the Admin to do something
		theStage.setTitle("CSE 360 Foundation Code: First User Account Setup");	
		theStage.setScene(passwordResetScene);
		theStage.show();
	}
	 ...
	 * 
	 */
	private ViewPasswordReset(String userName) {

		// Create the Pane for the list of widgets and the Scene for the window
		theRootPane = new Pane();
		passwordResetScene = new Scene(theRootPane, width, height);
		...
		setupTextUI(text_Password1, "Arial", 18, 300, Pos.BASELINE_LEFT, 50, 210, 
				true);
		text_Password1.setPromptText("Enter New Password");
		text_Password1.textProperty().addListener((observable, oldValue, newValue)
				-> {ControllerPasswordReset.setPassword1(); });

		// Establish the text input operand field for the password
		setupTextUI(text_Password2, "Arial", 18, 300, Pos.BASELINE_LEFT, 50, 260, 
				true);
		text_Password2.setPromptText("Enter New Password Again");
		text_Password2.textProperty().addListener((observable, oldValue, newValue) 
				-> {ControllerPasswordReset.setPassword2(); });

		// Set up the Log In button
		setupButtonUI(button_ResetPassword, "Dialog", 18, 200, Pos.CENTER, 475, 210);
		button_ResetPassword.setOnAction((event) -> {
			ControllerPasswordReset.setNewPassword(); 
			
			});

		setupLabelUI(canReturn, "Arial", 18, width, Pos.CENTER, 0, 130);
		...
		button_Quit.setOnAction((event) -> {ControllerPasswordReset.performQuit(); });

		// Place all of the just-initialized GUI elements into the pane
		theRootPane.getChildren().addAll(label_ApplicationTitle, label_TitleLine2, text_Username, text_Password1, 
				text_Password2, button_ResetPassword, label_PasswordsDoNotMatch,
				button_Quit);
	}
