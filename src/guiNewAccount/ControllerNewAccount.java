	...
	*/
	...
	protected static void doCreateUser() {
		...
		if (ViewNewAccount.text_Password1.getText().compareTo(ViewNewAccount.text_Password2.getText()) != 0) {
			...
			ViewNewAccount.alertPasswordNotMatchError.showAndWait();
			return;
		}
		
		String usernameError = userNameRecognizer.UserNameRecognizer.checkForValidUserName(username);
		String passwordError = passwordEvaluation.PasswordEvaluation.evaluatePassword(ViewNewAccount.text_Password1.getText());

		if (!usernameError.equals("")) {
			ViewNewAccount.text_Username.setText("");
			ViewNewAccount.alertBadUsernameError.setContentText(usernameError);
			ViewNewAccount.alertBadUsernameError.showAndWait();
			return;
		}
		...
		if (!passwordError.equals("")) {
			ViewNewAccount.text_Password1.setText("");
			ViewNewAccount.text_Password2.setText("");
			ViewNewAccount.alertBadPasswordError.setContentText(passwordError);
			ViewNewAccount.alertBadPasswordError.showAndWait();
			return;
		}
        ...
        // Navigate to the User Login Page to re-sign in
