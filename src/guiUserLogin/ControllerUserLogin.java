...
import guiPasswordReset.ViewPasswordReset;
	...
	protected static void doLogin(Stage ts) {
    	...
    	String actualPassword = theDatabase.getCurrentPassword();
    	 ...
    	 if (password.compareTo(actualPassword) != 0) {	
    		 //If the password is a valid one time password for the user, send them to the password reset page.
    		 if(theDatabase.validOTPCode(username, password)) {
    			ViewPasswordReset.displayPasswordReset(ts, username);
    			return;
    		 } else {
    	    		ViewUserLogin.alertUsernamePasswordError.setContentText(
    	     		"Incorrect username/password. Try again!");
