	...
	*/
	...
	protected static void doSetupAdmin(Stage ps, int r) {
		...
		if (adminPassword1.compareTo(adminPassword2) == 0 ) {
			...
			usernameError = userNameRecognizer.UserNameRecognizer.checkForValidUserName(adminUsername);
			passwordError = passwordEvaluation.PasswordEvaluation.evaluatePassword(adminPassword1);
			...
			if (usernameError.equals("")) {
				...
				if (passwordError.equals("")) {
		            ...
		            // First user(admin) was established in the database, so navigate to the User Login Page
		        	guiUserLogin.ViewUserLogin.displayUserLogin(ViewFirstAdmin.theStage);

				} else {
					ViewFirstAdmin.alertBadPasswordError.setTitle("Password Error");
					ViewFirstAdmin.alertBadPasswordError.setHeaderText("Invalid Password");
					ViewFirstAdmin.alertBadPasswordError.setContentText(passwordError);
					ViewFirstAdmin.alertBadPasswordError.showAndWait();
				}
			} else {
				ViewFirstAdmin.alertBadUsernameError.setTitle("Username Error");
				ViewFirstAdmin.alertBadUsernameError.setHeaderText("Invalid Username");
				ViewFirstAdmin.alertBadUsernameError.setContentText(usernameError);
				ViewFirstAdmin.alertBadUsernameError.showAndWait();
			}
